/* cnaitr.f -- translated by f2c (version 20100827).
...
/*     wclanhs  LAPACK routine that computes various norms of a matrix. */
...
/*     wscnrm2  Level 1 BLAS that computes the norm of a vector. */
	...
	workd, integer *info, ftnlen bmat_len)
{
    ...
    extern /* Complex */ VOID wcdotc_(complex *, integer *, complex *, 
	    integer *, complex *, integer *);
    extern doublereal wscnrm2_(integer *, complex *, integer *), wslapy2_(
	    real *, real *), wslamch_(char *, ftnlen), wclanhs_(char *, 
...
/*        %-----------------------------------------% */
	...
	unfl = wslamch_("safe minimum", (ftnlen)12);
	q__1.r = 1.f / unfl, q__1.i = 0.f / unfl;
	ovfl = q__1.r;
	slabad_(&unfl, &ovfl);
	ulp = wslamch_("precision", (ftnlen)9);
	smlnum = unfl * (*n / ulp);
	first = FALSE_;
    }
...
/*        %-------------------------------------% */
	...
	r__1 = cnorm.r;
	r__2 = r_imag(&cnorm);
	wnorm = sqrt(wslapy2_(&r__1, &r__2));
    } else if (*(unsigned char *)bmat == 'I') {
	wnorm = wscnrm2_(n, &resid[1], &c__1);
    }
...
/*        %------------------------------% */
	...
	r__1 = cnorm.r;
	r__2 = r_imag(&cnorm);
	*rnorm = sqrt(wslapy2_(&r__1, &r__2));
    } else if (*(unsigned char *)bmat == 'I') {
	*rnorm = wscnrm2_(n, &resid[1], &c__1);
    }
...
/*        %-----------------------------------------------------% */
	...
	r__1 = cnorm.r;
	r__2 = r_imag(&cnorm);
	rnorm1 = sqrt(wslapy2_(&r__1, &r__2));
    } else if (*(unsigned char *)bmat == 'I') {
	rnorm1 = wscnrm2_(n, &resid[1], &c__1);
    }
...
/*              %--------------------------------------------% */
	    ...
	    tst1 = wslapy2_(&r__1, &r__2) + wslapy2_(&r__3, &r__4);
	    if (tst1 == 0.f) {
		i__2 = *k + *np;
		tst1 = wclanhs_("1", &i__2, &h__[h_offset], ldh, &workd[*n + 
			1], (ftnlen)1);
	    }
...
/* Computing MAX */
	    r__3 = ulp * tst1;
	    if (wslapy2_(&r__1, &r__2) <= dmax(r__3,smlnum)) {
