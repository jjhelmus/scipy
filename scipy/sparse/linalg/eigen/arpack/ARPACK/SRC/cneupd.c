/* cneupd.f -- translated by f2c (version 20100827).
...
/*     wscnrm2  Level 1 BLAS that computes the norm of a complex vector. */
	...
	bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int cunm2r_(char *, char *, integer *, integer *, 
	    ...
	    complex *, integer *, ftnlen, ftnlen), csscal_(integer *, real *, 
    ...
    extern /* Subroutine */ int cngets_(integer *, char *, integer *, integer 
	    *, complex *, complex *, ftnlen), claset_(char *, integer *, 
	    integer *, complex *, complex *, complex *, integer *, ftnlen), 
	    ctrsen_(char *, char *, logical *, integer *, complex *, integer *
    ...
    extern doublereal wscnrm2_(integer *, complex *, integer *), wslapy2_(
	    real *, real *), wslamch_(char *, ftnlen);
...
/* Computing MAX */
	    ...
	    r__1 = eps23, r__2 = wslapy2_(&r__3, &r__4);
	    ...
	    if (numcnv < nconv && wslapy2_(&r__1, &r__2) <= *tol * rtemp) {
		select[jj] = TRUE_;
		++numcnv;
		if (jj > nconv) {
		    reord = TRUE_;
...
/*           %------------------------------------------------% */
	    ...
	    i__1 = nconv;
	    for (j = 1; j <= i__1; ++j) {
		rtemp = wscnrm2_(ncv, &workl[invsub + (j - 1) * ldq], &c__1);
		rtemp = 1.f / rtemp;
