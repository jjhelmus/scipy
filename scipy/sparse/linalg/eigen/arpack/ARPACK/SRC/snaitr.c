/* snaitr.f -- translated by f2c (version 20100827).
...
/*     wslanhs  LAPACK routine that computes various norms of a matrix. */
	...
	info, ftnlen bmat_len)
{
    ...
    static real smlnum;
    extern doublereal wslamch_(char *, ftnlen), wslanhs_(char *, integer *, 
...
/*        %-------------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	wnorm = wsdot_(n, &resid[1], &c__1, &workd[ipj], &c__1);
	wnorm = sqrt((dabs(wnorm)));
    } else if (*(unsigned char *)bmat == 'I') {
	wnorm = wsnrm2_(n, &resid[1], &c__1);
    }
...
/*        %------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	*rnorm = wsdot_(n, &resid[1], &c__1, &workd[ipj], &c__1);
	*rnorm = sqrt((dabs(*rnorm)));
    } else if (*(unsigned char *)bmat == 'I') {
	*rnorm = wsnrm2_(n, &resid[1], &c__1);
    }
...
/*        %-----------------------------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	rnorm1 = wsdot_(n, &resid[1], &c__1, &workd[ipj], &c__1);
	rnorm1 = sqrt((dabs(rnorm1)));
    } else if (*(unsigned char *)bmat == 'I') {
	rnorm1 = wsnrm2_(n, &resid[1], &c__1);
    }
...
/*              %--------------------------------------------% */
	    ...
	    if (tst1 == 0.f) {
		i__2 = *k + *np;
		tst1 = wslanhs_("1", &i__2, &h__[h_offset], ldh, &workd[*n + 
			1], (ftnlen)1);
	    }
/* Computing MAX */
	    r__2 = ulp * tst1;
