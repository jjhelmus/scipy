/* sneupd.f -- translated by f2c (version 20100827).
...
/*     wslamch  LAPACK routine that determines machine constants. */
/*     wslapy2  LAPACK routine to compute sqrt(x**2+y**2) carefully. */
...
/*     wsdot    Level 1 BLAS that computes the scalar product of two vectors. */
/*     wsnrm2   Level 1 BLAS that computes the norm of a vector. */
	...
	which_len)
{
    ...
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    ...
	    *, integer *, integer *, integer *, real *, integer *, real *, 
	    ...
	    integer *, integer *, integer *, ftnlen, ftnlen), sngets_(integer 
	    *, char *, integer *, integer *, real *, real *, real *, real *, 
	    real *, ftnlen);
    extern doublereal wslapy2_(real *, real *), wslamch_(char *, ftnlen);
...
/*     %---------------------------------% */
    ...
    eps23 = wslamch_("Epsilon-Machine", (ftnlen)15);
...
/*        %-----------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
/* Computing MAX */
	    r__1 = eps23, r__2 = wslapy2_(&workl[irr + *ncv - j], &workl[iri 
		    + *ncv - j]);
...
/*                 %----------------------% */
		    ...
		    temp = wsnrm2_(ncv, &workl[invsub + (j - 1) * ldq], &c__1)
			    ;
		    r__1 = 1.f / temp;
...
/*                 %-------------------------------------------% */
		    ...
		    if (iconj == 0) {
			r__1 = wsnrm2_(ncv, &workl[invsub + (j - 1) * ldq], &
				c__1);
			r__2 = wsnrm2_(ncv, &workl[invsub + j * ldq], &c__1);
			temp = wslapy2_(&r__1, &r__2);
			r__1 = 1.f / temp;
...
/*                 %-------------------------------------------% */
		    ...
		    if (iconj == 0) {
			workev[j] = wslapy2_(&workev[j], &workev[j + 1]);
			workev[j + 1] = workev[j];
			iconj = 1;
		    } else {
			iconj = 0;
...
/*        %---------------------------------------% */
	...
	if (s_cmp(type__, "SHIFTI", (ftnlen)6, (ftnlen)6) == 0) {
	    ...
	    i__1 = *ncv;
	    for (k = 1; k <= i__1; ++k) {
		temp = wslapy2_(&workl[iheigr + k - 1], &workl[iheigi + k - 1]
			);
...
/*        %------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
		...
		temp = wslapy2_(&workl[iheigr + j - 1], &workl[iheigi + j - 1]
			);
		workev[j] = (workl[invsub + (j - 1) * ldq + *ncv - 1] * workl[
