/* cumfnc.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int cumfnc_(doublereal *f, doublereal *dfn, doublereal *dfd, 
	doublereal *pnonc, doublereal *cum, doublereal *ccum)
{
    /* Initialized data */
...
/*     Now sum terms backward from icent until convergence or all done */
    ...
    d__1 = adn + b;
    d__2 = adn + 1.;
    dnterm = exp(alngam_(&d__1) - alngam_(&d__2) - alngam_(&b) + adn * log(xx)
	     + b * log(yy));
L30:
    d__1 = xmult * betdn;
...
/*     Now sum forwards until convergence */
	...
	d__1 = aup - 1 + b;
	upterm = exp(alngam_(&d__1) - alngam_(&aup) - alngam_(&b) + (aup - 1) 
		* log(xx) + b * log(yy));
    }
    goto L60;
L50:
